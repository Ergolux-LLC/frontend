...
export async function initTwilio(token, identity) {
  ...
  audioEls.forEach((el, i) => {
    console.log(`[twilio] <audio>[${i}] src: ${el.src || '(media stream)'}`);
    el.addEventListener('play', () => console.log(`[twilio] <audio>[${i}] started playing`));
    el.addEventListener('volumechange', () => console.log(`[twilio] <audio>[${i}] volume changed: ${el.volume}`));
    el.addEventListener('loadedmetadata', () => console.log(`[twilio] <audio>[${i}] metadata loaded`));
  });
}, 1000);
 ...
 // NOW begin polling for <audio> elements
  ...
  const pollAudioElements = () => {
    ...
    audioEls.forEach((el, i) => {
      ...
      console.log(`[twilio] [detect] New <audio> element #${i}, src: ${el.src || '(media stream)'}`);
      el.addEventListener('play', () => console.log(`[twilio] <audio> #${i} started playing`));
      el.addEventListener('volumechange', () => console.log(`[twilio] <audio> #${i} volume changed to: ${el.volume}`));
      el.addEventListener('loadedmetadata', () => console.log(`[twilio] <audio> #${i} metadata loaded`));
    });
  };
...
call.on('accept', () => {
  ...
  const poll = setInterval(() => {
    log(`[twilio] [poll] Mic gain: ${inputGain?.gain?.value}, Speaker gain: ${outputGain?.gain?.value}`);
    if (!activeCall) clearInterval(poll);
  }, 2000);

  window.dispatchEvent(new CustomEvent('twilio:callAccepted', { detail: { call } }));
});
